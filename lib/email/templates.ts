...
// ─── Magic-link email (server-initiated post-payment auth) ──────────
//
// We send this ourselves via Resend rather than relying on Supabase's
// built-in OTP email. Three reasons, all painful lessons from real
// customer tests:
//
//   1. PKCE/implicit flow choice keeps biting us. Supabase's
//      built-in signInWithOtp generates either a `?code=` link
//      (PKCE — needs a code_verifier cookie that doesn't exist when
//      the OTP was generated server-side from a webhook) or a
//      fragment-only link (`#access_token=...` — server callback
//      can't read fragments). Both broke in production. The
//      generateLink admin API gives us a `?token_hash=&type=magiclink`
//      shape that works with verifyOtp on the callback — period.
//
//   2. The default Supabase email is generic ("Sign in to your
//      account") and reads as transactional/bot. Customers who just
//      paid $9.99 expect a branded EvenQuote email; the generic one
//      gets misread as suspicious or "did the payment work?".
//
//   3. Supabase's outbound SMTP has its own deliverability profile,
//      separate from our Resend domain reputation. Single sending
//      surface = single inbox-reputation game = better deliverability
//      over time.

export type MagicLinkInput = {
  recipientName?: string | null;
  /**
   * The ready-to-click action link. Generated by
   * supabase.auth.admin.generateLink({type:'magiclink',...}); already
   * includes the token_hash and the redirect_to chain that lands the
   * user at our /auth/callback?next=... → claim → success page.
   */
  actionLink: string;
  /**
   * Vertical name for the subject line, e.g. 'Handyman'. Optional —
   * if omitted, subject falls back to a generic phrase. Helps the
   * customer's inbox sort + scan.
   */
  categoryName?: string | null;
};

export function renderMagicLink(input: MagicLinkInput): Rendered {
  const greeting = input.recipientName ? `Hi ${escapeHtml(input.recipientName)},` : 'Hi,';
  const cat = input.categoryName ? input.categoryName.toLowerCase() : 'quote';

  // Subject explicitly says "sign in" so it cannot be misread as
  // "your quotes are ready" — exactly the misread that happened on
  // 2026-05-01 with the default Supabase magic-link email.
  const subject = `Sign in to track your EvenQuote ${cat} request`;

  const inner = `
    <p style="font-size:16px; margin:0 0 12px 0;">${greeting}</p>
    <p style="font-size:15px; margin:0 0 16px 0; color:#374151;">
      Your payment went through and your ${escapeHtml(cat)} request is in the
      system. Click below to sign in to your EvenQuote account &mdash; this
      lets you watch progress and (later) release your contact info to any
      pro you want to follow up with.
    </p>
    ${button('Sign in to EvenQuote', input.actionLink)}
    <p style="font-size:13px; color:#6b7280; margin-top:24px;">
      The link is single-use and expires in 1 hour. If it doesn&rsquo;t
      work or expired, reply to this email and we&rsquo;ll send a fresh one.
    </p>
  ...
  const text = [
    greeting,
    ``,
    `Your payment went through and your ${cat} request is in the system. Click the link below to sign in to your EvenQuote account — this lets you watch progress and (later) release your contact info to any pro you want to follow up with.`,
    ``,
    `Sign in: ${input.actionLink}`,
    ``,
    `The link is single-use and expires in 1 hour. If it doesn't work or expired, reply to this email and we'll send a fresh one.`,
  ].join('\n');
