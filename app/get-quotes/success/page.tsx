...
//
// Race-condition note (flow A): Stripe's redirect back here can land a
// second or two before our /api/stripe/webhook POST inserts the payments
// row. Rather than 404 a paying customer in that window, we render a
// 'pending-webhook' card with a meta-refresh every 3s. The next render
// (once the webhook has landed) falls through to the normal confirmation.
//
// PII note: this page is behind a UUID or Stripe session_id in the URL,
// both of which are guessable-adjacent. We mask the contact_email before
// displaying it (see lib/text/pii.ts) so a URL-sharer can't harvest the
...
type ResolvedState =
  | { kind: 'not-found' }
  | { kind: 'pending-webhook'; sessionId: string }
  | {
      kind: 'ok';
      requestId: string;
      city: string;
...
async function resolveState(sp: Props['searchParams']): Promise<ResolvedState> {
  ...
  if (!requestId && sp.session_id) {
    // Look up the session in payments — it'll be there once the Stripe
    // webhook has inserted the payments row. Stripe's redirect back to
    // success can occasionally beat the webhook by a second or two; in
    // that window we DON'T 404 — we show a polling state that auto-refreshes
    // until the row lands. Returning not-found would give the user a
    // broken-looking post-payment page for a purely transient race.
    ...
    if (!requestId) {
      return { kind: 'pending-webhook', sessionId: sp.session_id };
    }
  }
...
export default async function SuccessPage({ searchParams }: Props) {
  ...
  // Transient race: Stripe has redirected the user here with a session_id,
  // but the /api/stripe/webhook POST hasn't landed yet (or is still in-flight).
  // Instead of 404'ing a paying customer, show a polling card that auto-
  // refreshes every 3s. Once the payments row exists, resolveState() will
  // resolve to 'ok' on the next render and the normal confirmation shows.
  if (state.kind === 'pending-webhook') {
    return (
      <>
        {/* next/head isn't available in server components inside app/;
            a plain meta tag in the body still triggers the browser's
        ...
        <main className="container max-w-2xl py-12 sm:py-16">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <span className="inline-flex items-center gap-2 rounded-full border-2 border-ink bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wider">
              <span className="h-2 w-2 animate-pulse rounded-full bg-ink" aria-hidden />
              Finalizing
            </span>
          ...
          <div className="relative overflow-hidden rounded-lg border-2 border-ink bg-card p-8 shadow-[6px_6px_0_0_hsl(var(--foreground))]">
            ...
            />
            <p className="label-eyebrow mb-3 relative z-10">Almost there</p>
            ...
            <p className="mt-3 text-muted-foreground relative z-10">
              Stripe tells us the payment went through — we&rsquo;re just waiting
              for their final confirmation ping. This page will refresh automatically
              every few seconds until it lands (usually under 10).
            </p>
            ...
            <div className="mt-6 rounded-md border-2 border-ink bg-white p-5 relative z-10">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">You&rsquo;re all paid up</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    No need to pay again. If this page is still spinning after a
                    minute, email{' '}
                    <a className="underline" href="mailto:support@evenquote.com">
                      support@evenquote.com
                    </a>{' '}
                    with the reference below and we&rsquo;ll sort it out personally.
                  </p>
                </div>
              </div>
            </div>
            ...
            <p className="mt-6 text-xs text-muted-foreground relative z-10">
              Stripe reference: <span className="font-mono">{state.sessionId}</span>
            </p>
          </div>
        </main>
        <SiteFooter />
          ...
          {isClaimedByViewer ? (
            ...
            <>
              <p className="mt-3 text-muted-foreground relative z-10">
                ...
                <span className="font-semibold text-foreground">
                  {maskEmail(state.email) ?? 'the email you provided'}
                </span>
