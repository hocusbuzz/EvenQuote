...
//
// Context: the call engine (lib/calls/engine.ts) flips a calls row to
// status='failed' when Vapi returns 4xx/5xx on the initial dispatch.
// Without a retry, a transient Vapi hiccup costs the customer a call
// slot permanently. This route picks up those rows and re-dispatches.
//
...
//
// Auth: requires CRON_SECRET header match. The easiest wiring is Supabase
// Scheduled Functions (pg_cron hitting this URL) or Vercel Cron — both
// pass through a custom header.
//
// Call this by: `curl -H 'x-cron-secret: <secret>' https://…/api/cron/retry-failed-calls`
// Returns a JSON summary, which is what Vercel Cron logs for you.
...
// Vercel Cron uses GET by default; accept both so the route works no
// matter which scheduler triggers it.
